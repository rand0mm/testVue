  ...
  constructor(config = {}) {
    ...
    const documentHeight = () => {
      ...
      const height =
        ...
        window.innerHeight
      doc.style.setProperty('--vh', `${height / 100}px`) // На мобилах всегда статична. Высота с учетом показа строки состояния
      doc.style.setProperty('--dvh', `${window.innerHeight / 100}px`) // На мобилах изменяется, в зависимости от показа строки состояния
    }
    const setIsInit = () => {

...
export default class PopupClass {
  ...
  static manualInit(popup) {
    if (popup) {
      ...
      const emptyResults = popup.querySelector(`.${EMPTY_RESULTS_CLASS}`)
      ...
      const fetchData = (openBtn) => {
        if (openBtn && dynamicContent) {
          ...
          if (url) {
            ...
            if (emptyResults) {
              emptyResults.classList.remove(SHOW_CLASS_NAME)
            }
            ...
            window.app.http
              ...
              .then((res) => {
                ...
                if (res?.object?.html) {
                  ...
                  emptyResults.classList.add(SHOW_CLASS_NAME)
                }
      ...
      const toggleOpen = (open = false, openBtn) => {
        ...
        if (open) {
          ...
          if (!window.app.body.classList.contains(OVERFLOW_HIDDEN)) {
            setTimeout(() => {
          ...
          if (openBtn && !openBtn.classList.contains(POPUP_FETCH_ONLY_BTN_CLASS)) {
